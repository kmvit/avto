...
class RuleList(ListView):
    model = Rules_category
    
class RoadList(ListView):
    model = Road_category

class RoadSingList(ListView):
    model = Roadsing_category

class RoadSingDetail(DetailView):
    model = Roadsing_category
    
    def get_context_data(self, **kwargs):
        context = super(RoadSingDetail, self).get_context_data(**kwargs)
        context['category_list'] = Roadsing_category.objects.all()
        context['road_list'] = Roadsing.objects.filter(category__slug=self.kwargs['slug']).order_by('id')
        return context
        
def RegulView(request, slug):
    regul = Regul.objects.get(slug=slug)
    context = {'regul':regul}
    return render(request, 'quiz/regul.html', context)